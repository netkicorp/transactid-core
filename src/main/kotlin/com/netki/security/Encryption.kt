...
import com.netki.security.Parameters.MAXIMUM_LENGTH_PAD_ENCRYPTION
import com.netki.security.Parameters.UPPER_HEX_DIGITS_ENCRYPTION
...
object Encryption {
    ...
    fun encrypt(
        ...
        try {
            val publicKeyReceiver = publicKeyReceiverPem.toPublicKey() as ECPublicKey
            val publicKeySender = publicKeySenderPem.toPublicKey() as ECPublicKey
            val privateKeySender = privateKeySenderPem.toPrivateKey()
    ...
    private fun leftPadWithZeroes(originalString: String): String {
        ...
        while (paddedString.length < MAXIMUM_LENGTH_PAD_ENCRYPTION) {
    ...
    private fun encode(bytes: ByteArray): String {
        ...
        for (cur in bytes) {
            stringBuilder.append(UPPER_HEX_DIGITS_ENCRYPTION[cur.toInt() shr 4 and 0xF])
            stringBuilder.append(UPPER_HEX_DIGITS_ENCRYPTION[cur.toInt() and 0xF])
        }
